...
type UserOperationsAPIController struct {
	service      UserOperationsAPIServicer
...
func WithUserOperationsAPIErrorHandler(h ErrorHandler) UserOperationsAPIOption {
	return func(c *UserOperationsAPIController) {
		c.errorHandler = h
	}
}
...
func NewUserOperationsAPIController(s UserOperationsAPIServicer, opts ...UserOperationsAPIOption) Router {
	controller := &UserOperationsAPIController{
		service:      s,
...
func (c *UserOperationsAPIController) GetUser(w http.ResponseWriter, r *http.Request) {
	...
	userSlugParam := chi.URLParam(r, "user_slug")
	if userSlugParam == "" {
		c.errorHandler(w, r, &RequiredError{"user_slug"}, nil)
		return
	}
	...
	result, err := c.service.GetUser(r.Context(), userSlugParam, withHtmlParam, includeFieldsParam, excludeFieldsParam, ifModifiedSinceParam)
...
func (c *UserOperationsAPIController) GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	ifModifiedSinceParam := r.Header.Get("If-Modified-Since")
	result, err := c.service.GetUsers(r.Context(), limitParam, offsetParam, sortByParam, withHtmlParam, includeFieldsParam, excludeFieldsParam, ifModifiedSinceParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
