...
func AssertModuleCurrentReleaseRequired(obj ModuleCurrentRelease) error {
	if err := AssertReleaseModuleRequired(obj.Module); err != nil {
		return err
	}
	for _, el := range obj.Tasks {
		if err := AssertReleaseTaskRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Plans {
		if err := AssertReleasePlanAbbreviatedRequired(el); err != nil {
			return err
		}
	}
	return nil
...
func AssertModuleCurrentReleaseConstraints(obj ModuleCurrentRelease) error {
	if obj.ValidationScore < 0 {
		return &ParsingError{Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.ValidationScore > 100 {
